...
class stackRPN():
	...
	def __init__(self):
		...
		self.expr_types = enum.Enum("expr_types","arithmetic stackop function constant")
		self.arithmetic = 0
		self.stackop = 1
		self.function = 2
		self.constant = 3
		...
		'sin'     : [ math.sin, self.expr_types.function, 1 ],
		'cos'     : [ math.cos, self.expr_types.function, 1 ],
		'tan'     : [ math.tan, self.expr_types.function, 1 ],
		'asin'    : [ math.asin, self.expr_types.function, 1 ],
		'acos'    : [ math.acos, self.expr_types.function, 1 ],
		'atan'    : [ math.atan, self.expr_types.function, 1 ],
		'sqrt'    : [ math.sqrt, self.expr_types.function, 1 ],
		'exp'     : [ self.stackEXP, self.expr_types.arithmetic, 2 ],
		'^'       : [ self.stackEXP, self.expr_types.arithmetic, 2 ],
		'deg'     : [ math.degrees, self.expr_types.function, 1 ],
		'rad'     : [ math.radians, self.expr_types.function, 1 ],
		'log'     : [ math.log, self.expr_types.function, 1 ],
		'ln'      : [ math.log, self.expr_types.function, 1 ],
		'+'       : [ self.stackAdd, self.expr_types.arithmetic, 2 ],
		'*'       : [ self.stackMult, self.expr_types.arithmetic, 2 ],
		'-'       : [ self.stackSubtract, self.expr_types.arithmetic, 2 ],
		'/'       : [ self.stackDiv, self.expr_types.arithmetic, 2 ],
		'neg'     : [ self.stackNEG, self.expr_types.stackop, 1 ],
		'n'       : [ self.stackNEG, self.expr_types.stackop, 1 ],
		'inv'     : [ self.stackINV, self.expr_types.arithmetic, 1 ],
		'pi'      : [ (lambda : math.pi), self.expr_types.constant, 0 ],
		'e'       : [ (lambda : math.e), self.expr_types.constant, 0 ],
		'swap'    : [ self.stackSWAP, self.expr_types.stackop, 2 ],
		's'       : [ self.stackSWAP, self.expr_types.stackop, 2 ],
		'drop'    : [ self.stackDROP, self.expr_types.stackop, 1 ],
		'dr'      : [ self.stackDROP, self.expr_types.stackop, 1 ],
		'dd'      : [ self.stackDROP, self.expr_types.stackop, 1 ],
		'dropn'   : [ self.stackDROPN, self.expr_types.stackop, 1 ],
		'dup'     : [ self.stackDUP, self.expr_types.stackop, 1 ],
		'd'      : [ self.stackDUP, self.expr_types.stackop, 1 ],
		'rot'     : [ self.stackROT, self.expr_types.stackop, 3 ],
		'clean'   : [ self.stackCLEAN, self.expr_types.stackop, 0 ],
		'ddup'    : [ self.stackDDUP, self.expr_types.stackop, 2 ],
		'dupn'    : [ self.stackDUPN, self.expr_types.stackop, 1 ],
		'help'    : [ self.stackHELP, self.expr_types.stackop, 0 ],
		"exit"    : [ self.stackEXIT, self.expr_types.stackop, 0 ],
		"quit"    : [ self.stackEXIT, self.expr_types.stackop, 0 ]
		}
	...
	def cmd(self,c):
		...
		try:
			if ctype == self.expr_types.function:
				...
				#For some reason, None appears in the stack sometimes, so clean it
